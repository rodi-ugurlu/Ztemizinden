...
const originalDirectGrants = Boolean(webClient.directAccessGrantsEnabled);
...
assert(originalDirectGrants === false, 'Direct access grants must be disabled.');
assert(webClient.attributes?.['pkce.code.challenge.method'] === 'S256', 'PKCE S256 must be required.');
const checks = [];
let result;
try {
  await adminRequest(
    `/admin/realms/${encodeURIComponent(realm)}/clients/${encodeURIComponent(webClient.id)}`,
    { method: 'PUT', body: { ...webClient, directAccessGrantsEnabled: true } }
  );
  ...
  await adminRequest(
    `/admin/realms/${encodeURIComponent(realm)}/clients/${encodeURIComponent(webClient.id)}`,
    { method: 'PUT', body: { ...webClient, directAccessGrantsEnabled: originalDirectGrants } }
  );
}

const restoredWebClients = await adminRequest(
  `/admin/realms/${encodeURIComponent(realm)}/clients?clientId=ztemizinden-web`
);
assert(
  restoredWebClients[0]?.directAccessGrantsEnabled === false,
  'Direct access grants were not restored to disabled.'
);
console.log(JSON.stringify({ ...result, pkce: 'S256', directAccessGrantsEnabled: false }));
